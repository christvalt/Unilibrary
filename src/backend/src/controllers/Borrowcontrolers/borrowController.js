...
const borrowBook = (req, res, next) => {
  ...
  var foundBook = new borrow({
    ...
    returningdate: new Date(Date.now() + 3 * 24 * 60 * 60 * 1000),
    actualReturnDate: Date.now(),
  });
  console.log(req.body.userId);

  foundBook.save().then((updatedBorrowedBook) => {
...
const returnBook = (req, res) => {
  ...
  Boorow.update(
    ...
    { returned: true, actualReturnDate: Date.now(), returningdate: Date.now() },
    function (err, book) {
      if (err) res.send(err);
      else {
    ...
    .aggregate([
      {
        $match: {
          $and: [
            { returned: false },
            { returningdate: { $lt: new Date(Date.now()) } },
          ],
        },
      },
      {
